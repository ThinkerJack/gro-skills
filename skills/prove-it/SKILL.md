...
当项目已有环境配置、结构化日志、trace、真实服务探针或端到端验证设施时:

1. 先从仓库代码和文档发现可用的运行方式、环境配置和验证入口。
2. 默认验证链不要隐式访问外网、真机、付费资源或生产系统;真实集成探针必须显式 opt-in。
3. 运行时证据要可复查且脱敏,优先复用项目已有日志、trace、报告或约定目录。
4. 不要只贴控制台最后一行;用结构化日志、trace、截图、报告或摘要证明关键链路。
5. token、secret、cookie、Authorization、refresh token 等敏感原值不得写进日志、
   报告或最终回复;只记录存在性、状态码、类型和非敏感业务状态。
6. 跳过真实探针时必须真正停止执行,不要让后续步骤继续触发外部副作用。

复杂运行时验证可按需读取 `references/runtime-proof.md`。
