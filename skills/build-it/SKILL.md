...
`ship` **不合并、不推送基线分支**。合并回基线或开 PR 是另一步：`commit-it finish`。
...
**只有**`通过`**才继续**，按 `commit-it done` 的规矩语义分组提交并推送功能分支。其余三种停下报告，如实说出结论，不改写成「基本可用」。

## 冲突时停下来

上游文档、代码事实、用户最新要求三者打架时，**说出来再继续**——不要挑一个默默执行。

需要自行补齐会改变行为边界的决策时同样停下：那说明规格缺了东西，回到 `write-spec` 或 `make-plan`；缺的是技术方案或选型则回 `design-it`。
...
```
实现了什么  从用户或系统行为描述，不按文件复述
怎么实现      2—5 条关键结构或数据流
为什么这样  只写真正存在取舍的决定
偏离计划      变化和原因；没有就写「无」
请重点看      最值得人工检查的 1—3 个位置或行为
```

技术明细再列：改动文件 · 实际命令与结果 · 证据路径 · 未覆盖的风险和剩余事项。
