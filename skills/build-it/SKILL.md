...
实现完成后自己接着走完验证与提交，不再等你开口。

**验证**（`prove-it` 是手动 skill，这里够不到它，所以判据内联在此）：

1. 跑计划或规格里定义的验证命令——不是随手挑一个测试
2. 读完整输出，看退出码，数失败条目
3. 给出四态之一：

| 结论 | 含义 | auto 的动作 |
|---|---|---|
| `通过` | 所有必需检查都跑了且都过 | 继续提交推送 |
| `失败` | 跑了，发现错误行为 | **停下报告** |
| `部分验证` | 跑过的都过，但有必需检查没跑 | **停下报告** |
| `阻塞` | 环境或依赖不可用，跑不了 | **停下报告** |

**只有`通过`才继续。** 这一条没有例外——auto 授权的是「验证通过后自动提交」，不是「跳过验证直接提交」。后三种如实说出结论，不改写成「基本可用」。

**提交**：按 `commit-it done` 的规矩执行——分组、显式 pathspec、推送；在 worktree 里则继续合并回基线并清理。
